...
public:
  explicit VirtualSerialNode(const rclcpp::NodeOptions &options) : Node("serial_driver", options) {
    ...
    // aim_v2 is launched with node name "gimbal_pipeline" for SetMode compatibility.
    auto autoaim_set_mode_client =
      this->create_client<rm_interfaces::srv::SetMode>("gimbal_pipeline/set_mode");
    set_mode_clients_.emplace(autoaim_set_mode_client->get_service_name(),
                              autoaim_set_mode_client);
