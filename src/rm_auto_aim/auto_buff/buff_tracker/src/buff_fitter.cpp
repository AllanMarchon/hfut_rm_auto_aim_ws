...
auto_buff::BuffFitter::BuffFitter(rclcpp::Logger logger,
                                  const BuffFitterConfig &config)
    : logger_(std::move(logger)), config_(config) {
  this->reset();
  this->fitting_thread_ = std::thread{[this]() {
    RCLCPP_INFO(logger_, "[BuffFitter]: Fitting thread start!");
    while (running_.load(std::memory_order_relaxed)) {
      ...
      RCLCPP_DEBUG(logger_, "[BuffFitter]: Fitting time: %ld ms",
      ...
      if (result_opt.has_value()) {
        ...
        this->fitting_ok_ = true;
        RCLCPP_DEBUG(logger_, "[BuffFitter]: Fitting parameter updated!");
      }
      std::this_thread::sleep_for(
          std::chrono::milliseconds{config_.curve_fitting_interval_time_ms});
    }
    RCLCPP_INFO(logger_, "[BuffFitter]: Fitting thread stop!");
  }};
}

auto_buff::BuffFitter::~BuffFitter() {
  stop();
...
void auto_buff::BuffFitter::stop() {
